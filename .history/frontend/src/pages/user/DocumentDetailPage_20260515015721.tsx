...
export const DocumentDetailPage: React.FC<DocumentDetailPageProps> = ({ token }) => {
  ...
  const manualPassCount = document.pages?.filter((p) => (p.status || "").toString().toLowerCase() === "manual_pass").length || 0;
  const manualFailCount = document.pages?.filter((p) => (p.status || "").toString().toLowerCase() === "manual_fail").length || 0;
