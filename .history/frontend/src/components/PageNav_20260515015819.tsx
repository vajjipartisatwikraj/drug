...
export function PageNav({ markdown, status }: PageNavProps) {
  ...
  const passCount = pages.filter((p) => (p.status || "").toString().toLowerCase() === "pass").length;
  const failCount = pages.filter((p) => (p.status || "").toString().toLowerCase() === "fail").length;
  const voidCount = pages.filter((p) => (p.status || "").toString().toLowerCase() === "void").length;
