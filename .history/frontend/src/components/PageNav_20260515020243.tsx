...
export function PageNav({ markdown, status }: PageNavProps) {
  ...
  const pages = useMemo<PageInfo[]>(() => {
    // If an authoritative pages prop is passed, prefer it over parsing markdown.
    // This keeps counts consistent with backend data (document.pages).
    if ((arguments as any)[0] && (arguments as any)[0].pages) {
      // NOTE: TypeScript can't infer here; we'll handle pages when used by caller.
    }
